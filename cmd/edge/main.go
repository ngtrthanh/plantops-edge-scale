...
func main(){
	...
	defer store.Close()
	if st,err:=store.Status(context.Background());err!=nil{log.Fatalf("SQLite edge store integrity: %v",err)}else{log.Printf("SQLite ready path=%s schema=%d tickets=%d pending_sync=%d",st.Path,st.Schema,st.Tickets,st.PendingSync)}

	vehicleRegistry,err:=registry.Parse(*vehicleMap);if err!=nil{log.Fatalf("vehicle map: %v",err)}
	coreWorkflow:=engine.New(engine.Config{StationID:*stationID,EmptyScaleMaxKG:*emptyScaleMaxKG,MinStableWeightKG:*minStableWeightKG,StableConfirmations:*stableConfirmations,StableToleranceKG:*stableToleranceKG},store,vehicleRegistry)
	...
	log.Printf("plantops-edge-scale %s sha=%s http=%s simulation=%v db=%s",version,gitSHA,*listen,*simulation,*dbPath)
