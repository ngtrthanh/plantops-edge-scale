...
func main() {
	...
	coreWorkflow := engine.New(engine.Config{
		StationID: *stationID,
	...
	workflow := &workflowaudit.Recorder{Engine:coreWorkflow, Audit:eventAudit, StationID:*stationID, RuntimeGitSHA:gitSHA}
	...
	if *scaleAddr != "" {
		collector := &scaleascii.StreamCollector{
			Addr:*scaleAddr, StationID:*stationID, TransactionID:workflow.ActiveTransactionID,
			Journal:weightAudit, ReconnectDelay:*reconnectDelay,
			...
			OnFault:func(err error){
				...
				_ = workflow.ObserveFault(context.Background(),domain.Fault{Device:domain.DeviceScale,Health:domain.HealthFault,Reason:err.Error(),Overridable:false,Critical:true})
			},
		}
		go func(){
			if err:=collector.Run(context.Background());err!=nil{
				scaleMonitor.Fault(err)
				_ = workflow.ObserveFault(context.Background(),domain.Fault{Device:domain.DeviceScale,Health:domain.HealthFault,Reason:"raw weight collector stopped: "+err.Error(),Overridable:false,Critical:true})
	...
	if *ioAddr != "" {
		...
		auditedOutputs:=&runtimeio.AuditedOutputs{
			Inner:hardwareIO, Audit:eventAudit, StationID:*stationID, RuntimeGitSHA:gitSHA,
		...
		controller:=&runtimeio.Controller{Workflow:workflow,Inputs:hardwareIO,Outputs:auditedOutputs,Monitor:ioMonitor,PollInterval:*ioPoll,BuzzerPulse:*buzzerPulse,BarrierFeedbackTimeout:*barrierFeedbackTimeout}
		go func(){if err:=controller.Run(context.Background());err!=nil{log.Printf("CRITICAL runtime I/O controller stopped: %v",err)}}()
	...
	s := &httpapi.Server{
		RFID:rfid,LPR:lpr,WeightAudit:weightAudit,EventAudit:eventAudit,
		ScaleMonitor:scaleMonitor,Workflow:workflow,
		IOStatus:func()any{return ioMonitor.Snapshot()},AllowSimulation:*simulation,
