...
// action executes first; audit is best-effort afterward.
func (a *AuditedOutputs) SafeState(ctx context.Context) error {
	...
	if err := a.append(ctx, domain.AuditEvent{
...
func (a *AuditedOutputs) command(ctx context.Context, device domain.DeviceID, action string, permissive bool, execute func() error) error {
	...
	if permissive {
		if err := a.append(ctx,intent); err != nil { a.auditFailure(err); return AuditGateError{Err:err} }
	}
	...
	if auditErr := a.append(ctx,result); auditErr != nil {
		a.auditFailure(auditErr)
		if permissive && err == nil { return AuditGateError{Err:auditErr} }
	}
	return err
}
...
func (a *AuditedOutputs) auditFailure(err error) {
	if err != nil && a.OnAuditFailure != nil { a.OnAuditFailure(err) }
}
