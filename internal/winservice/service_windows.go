...
func Run(fn RunFunc) error {
	...
	// Windows SCM starts services with an implementation-defined working
	// directory. Make relative data/log paths deterministic: next to the EXE.
...
// Install creates an automatic delayed-start service. args are the application
// arguments persisted by SCM and supplied to the EXE at every boot.
func Install(args []string) error {
	...
	if existing, err := m.OpenService(Name); err == nil {
		existing.Close(); return fmt.Errorf("service %s already exists", Name)
	}
	...
	defer s.Close()
	// Restart on the first three failures; reset the failure count after a day.
	...
	if err := s.SetRecoveryActionsOnNonCrashFailures(true); err != nil {
		_ = s.Delete()
		return fmt.Errorf("set service recovery flag: %w", err)
	}
	return nil
}
...
func Uninstall() error {
	s, m, err := open(); if err != nil { return err }; defer m.Disconnect(); defer s.Close()
	st, qerr := s.Query()
	if qerr == nil && st.State != svc.Stopped { return fmt.Errorf("service %s must be stopped before uninstall", Name) }
	return s.Delete()
}
...
// StripManagementArg removes only -service itself while preserving all runtime
// configuration arguments for SCM. Both "-service install" and
// "-service=install" forms are handled.
