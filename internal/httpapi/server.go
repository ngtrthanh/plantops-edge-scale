...
func (s *Server) Handler() http.Handler {
	...
	mux.HandleFunc("GET /healthz", func(w http.ResponseWriter, _ *http.Request) {
		payload := map[string]any{
			...
			"event_audit_configured": s.EventAudit != nil,
		}
	...
	mux.HandleFunc("GET /api/io/status", func(w http.ResponseWriter, _ *http.Request) {
		if s.IOStatus == nil { writeJSON(w,http.StatusOK,map[string]any{"enabled":false}); return }
		writeJSON(w,http.StatusOK,s.IOStatus())
	})
	mux.HandleFunc("GET /api/scale/status", func(w http.ResponseWriter, _ *http.Request) {
		if s.ScaleMonitor == nil { writeJSON(w,http.StatusOK,map[string]any{"enabled":false}); return }
	...
	mux.HandleFunc("GET /api/audit/weights", func(w http.ResponseWriter, r *http.Request) {
		if s.WeightAudit == nil { writeJSON(w,http.StatusServiceUnavailable,map[string]string{"error":"raw weight audit not configured"}); return }
	...
	mux.HandleFunc("GET /api/audit/weights/verify", func(w http.ResponseWriter, _ *http.Request) {
		if s.WeightAudit == nil { writeJSON(w,http.StatusServiceUnavailable,map[string]string{"error":"raw weight audit not configured"}); return }
		writeVerify(w,s.WeightAudit.Verify())
	})
	mux.HandleFunc("GET /api/audit/events", func(w http.ResponseWriter, r *http.Request) {
		if s.EventAudit == nil { writeJSON(w,http.StatusServiceUnavailable,map[string]string{"error":"operational event audit not configured"}); return }
		limit:=parseLimit(r,200)
		records,err:=s.EventAudit.Tail(limit)
		if err!=nil { writeJSON(w,http.StatusInternalServerError,map[string]string{"error":err.Error()}); return }
		writeJSON(w,http.StatusOK,map[string]any{"count":len(records),"records":records})
	})
	mux.HandleFunc("GET /api/audit/events/verify", func(w http.ResponseWriter, _ *http.Request) {
		if s.EventAudit == nil { writeJSON(w,http.StatusServiceUnavailable,map[string]string{"error":"operational event audit not configured"}); return }
		writeVerify(w,s.EventAudit.Verify())
	})
	...
	mux.HandleFunc("POST /io/rfid", func(w http.ResponseWriter, r *http.Request) {
		...
		if json.NewDecoder(r.Body).Decode(&in)!=nil || in.Tag=="" { writeJSON(w,http.StatusBadRequest,map[string]string{"error":"tag required"}); return }
		s.RFID.Ingest(in.Tag,in.Quality); writeJSON(w,http.StatusAccepted,map[string]string{"status":"accepted"})
	})
	mux.HandleFunc("POST /io/lpr", func(w http.ResponseWriter, r *http.Request) {
		...
		s.LPR.Ingest(in.Plate,in.Confidence,in.ImageRef); writeJSON(w,http.StatusAccepted,map[string]string{"status":"accepted"})
	})
	...
	if s.AllowSimulation && s.Workflow != nil {
		mux.HandleFunc("POST /sim/position", func(w http.ResponseWriter, r *http.Request) {
			...
			if err:=json.NewDecoder(r.Body).Decode(&p); err!=nil { writeJSON(w,http.StatusBadRequest,map[string]string{"error":err.Error()}); return }
			if p.Observed.IsZero(){p.Observed=time.Now().UTC()}
			if err:=s.Workflow.ObservePosition(r.Context(),p); err!=nil { writeJSON(w,http.StatusConflict,map[string]string{"error":err.Error()}); return }
		...
		mux.HandleFunc("POST /sim/fault", func(w http.ResponseWriter, r *http.Request) {
			var f domain.Fault
			if err:=json.NewDecoder(r.Body).Decode(&f); err!=nil || f.Device=="" { writeJSON(w,http.StatusBadRequest,map[string]string{"error":"valid device fault required"}); return }
			if err:=s.Workflow.ObserveFault(r.Context(),f); err!=nil { writeJSON(w,http.StatusConflict,map[string]string{"error":err.Error()}); return }
			writeJSON(w,http.StatusAccepted,s.Workflow.Snapshot())
		})
		mux.HandleFunc("POST /sim/override", func(w http.ResponseWriter, r *http.Request) {
			var o domain.Override
			if err:=json.NewDecoder(r.Body).Decode(&o); err!=nil { writeJSON(w,http.StatusBadRequest,map[string]string{"error":err.Error()}); return }
			if err:=s.Workflow.AuthorizeOverride(r.Context(),o); err!=nil { writeJSON(w,http.StatusConflict,map[string]string{"error":err.Error()}); return }
			writeJSON(w,http.StatusAccepted,s.Workflow.Snapshot())
		})
		mux.HandleFunc("POST /sim/reset-complete", func(w http.ResponseWriter, _ *http.Request) {
			if err:=s.Workflow.ResetCompleted(); err!=nil { writeJSON(w,http.StatusConflict,map[string]string{"error":err.Error()}); return }
			writeJSON(w,http.StatusOK,s.Workflow.Snapshot())
		})
	}
...
func writeVerify(w http.ResponseWriter, err error) {
	if err==nil { writeJSON(w,http.StatusOK,map[string]any{"status":"ok","verified":true}); return }
	if os.IsNotExist(err) { writeJSON(w,http.StatusOK,map[string]any{"status":"empty","verified":true}); return }
